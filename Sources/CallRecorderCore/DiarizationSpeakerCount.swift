...
/// The number of voices to ask the detector for, and when to let it decide.
///
/// The detector counts voices on its own, and on the calls this app records it counts one too many:
/// a thirty-four minute standup with fourteen remote voices came back as sixteen, and the two extra
/// voices had to be named with a name the transcript already used somewhere else. The count is a
/// fact the app holds — the people on the call, less the person recording — and the detector answers
/// exactly that number when it is given one. Measured on the same recording: fourteen voices in
/// eighty-six seconds, against sixteen in a hundred and forty-four.
///
...
/// speech for that many people to have spoken at all, and the whole behaviour is one setting, so a
/// library whose participant lists are often incomplete can leave the count to the detector.
