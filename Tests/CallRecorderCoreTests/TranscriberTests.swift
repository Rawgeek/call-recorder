...
struct TranscriberTests {
    @Test("a finalized call stores participants and normalized transcript files", .enabled(if: TestEnvironment.hasFFmpeg), .enabled(if: TestEnvironment.hasVADModel))
    ...
    @Test("the glossary and the names are the prompt whisper is given", .enabled(if: TestEnvironment.hasFFmpeg), .enabled(if: TestEnvironment.hasVADModel))
    ...
    @Test("independent sources are transcribed and local microphone is attributed", .enabled(if: TestEnvironment.hasFFmpeg), .enabled(if: TestEnvironment.hasVADModel))
